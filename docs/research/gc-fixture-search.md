...
|---|---|---|---|---|---|---|
| [YOUNG IN Chromass Track C](youngin-chromass-format-investigation.md) | YOUNG IN Chromass and DataApex official material plus owner-supplied local intake | Exact-profile FID/TCD scientific signals for 9.0.1.19 and 9.1.0.76 | 30 local-only structural PRMs plus 27 same-run PRM/curve/Result pairs | Yes, local-only completed files | 30 files / 353,830 B smallest | Shared time/numeric-response semantics validated over 454,220 points; exact marker profiles add opt-in Ordifile-derived experimental RT/`calculated_area`; vendor Result equivalence is not claimed |
...
| [Pinned LabSolutions result ASCII `ladder.txt`](https://github.com/ethanbass/chromConverter/blob/9137b85f341ceb4f2bc71cc171650af75449ac96/tests/testthat/testdata/ladder.txt) | chromConverter repository; pinned external fixture | Shimadzu LabSolutions 5.82, GC-2014; FID | Standalone result ASCII, one `Peak Table(Ch1)` / `SFID1`, paired same-run GCD | No | 971,258 B / same | Exact SHA-256 and full 83-row RT/start/end/area/height digests; repository-level GPL >= 3 without file notice and embedded private metadata require controlled external handling |
...
| [Cambridge Apollo chromatography corpus](https://doi.org/10.17863/CAM.108306) | University of Cambridge; DOI dataset | Shimadzu LabSolutions 5.71 SP2/5.86; BID | 640 `.GCD`, 706 text, 636 same-directory stem pairs inspected | Yes | 37,512,839 B archive / member-level | CC BY 4.0; exact archive SHA-256 pinned in `shimadzu-gcd-corpus-external-fixture-manifest.json`; 320 stored peak tables decoded and 1,548 rows agreed with the paired text exports; embedded Windows paths and two addresses require controlled external handling |
...
| [Grob Mix GC-FID replicates](https://doi.org/10.5281/zenodo.19946728) | University of Duisburg-Essen / Zenodo | Agilent 6890N; FID | Raw and integration outputs / v179 `.ch` plus CSV | Yes | 481,259,756 B / 438,136 B `.ch` plus siblings | CC BY 4.0; ZIP64 central directory read by HTTP range request without downloading the archive: 96,446 members, 2,679 `.ch`, 2,677 `.D` run directories across 159 folders. One v179 `.ch` and its sibling report CSVs were fetched by range: the `.ch` carries no peak table, and the vendor peak rows live only in a sibling UTF-16LE headerless seven-column CSV whose filename is site-specific. Useful raw/export pairing, but attribution and privacy keep it external |
...
|---|---|---|---|---|---|---|
| YOUNG IN Chromass / YL-Clarity / Autochro | Proprietary CDS terms; local `.prm` redistribution is not authorized and no public reader specification was found | Product names only for factual compatibility research; no affiliation claim | Filename/sample and local-path or operator-field candidates observed; values remain local-only; credential patterns absent | No public source `.prm` or Autochro reader found; a proprietary OpenChrom converter listing is not an implementation basis | Independently written parser from owner-file facts and public docs; no proprietary/GPL code reused | Exact YL-Clarity 9.0.1.19 and 9.1.0.76 scientific signals plus opt-in marker-derived experimental `calculated_area` and separate explicit Result Tables; compatible unvalidated 9.x response units remain unresolved; vendor Result equivalence is not claimed |
...
| `ladder.txt` | Repository-level GPL >= 3; no file-specific notice found | Source repository, exact commit and unchanged-file statement | Personal and machine-local source metadata are embedded | Fixture schema/digests and official Peak Table semantics are evidence; GPL reader/test code is not reused | Standard-library independent parser; no GPL dependency or code | `ACCEPT_CONTROLLED_CI`; exact Experimental LabSolutions 5.82 result profile only, never committed/logged/uploaded |
...
### Shimadzu LabSolutions 5.82 GC-FID result ASCII

- Source: [pinned external `ladder.txt`](https://github.com/ethanbass/chromConverter/blob/9137b85f341ceb4f2bc71cc171650af75449ac96/tests/testthat/testdata/ladder.txt)
- Size: 971,258 bytes
- SHA-256: `46d1dcde188d7844c32abb89cda1f0d773cac480f6d6c93f2b6ca7149fdb9297`
- License: repository-level GPL >= 3; no file-specific notice found
- Privacy: embedded personal and local-machine text; external controlled-CI only
- Result: all 83 source Peak Table rows agree with canonical RT/start/end/area/height
  digests; RT boundaries are minutes and area/height units remain unset.
- Scope: standalone exact LabSolutions 5.82 / GC-2014 / one `Ch1` / `SFID1` result
  export only.

The workflow downloads this ASCII and the paired CC0 GCD into a private job-local
cache, validates both exact hashes and the full same-run relationship, uploads no
source or workbook artifact, and unconditionally removes both files. GPL source and
test expressions are not copied into Ordifile.
...
YOUNG IN Chromass was included as a mandatory priority candidate. Local intake supplies
25 YL-Clarity 9.0.1.19 structural PRMs and five 9.1.0.76 PRMs. Twelve independent 9.0
FID+TCD full-curve pairs validate 316,220 points; five 9.1 pairs validate 138,000. The
two validated profiles share zero-origin `DStep/MinTicks` minutes and identity numeric
response, while units remain 9.0 FID mV/TCD mV and 9.1 FID pA/TCD mV. Compatible
unvalidated 9.x files require the complete scientific fingerprint and retain unresolved
response units. Direct PRM peaks remain unsupported. See
