...
|---|---|---|---|---|---|---|
| [YOUNG IN Chromass Track C](youngin-chromass-format-investigation.md) | YOUNG IN Chromass and DataApex official material plus owner-supplied local intake | Exact-profile FID/TCD signals for 9.1; stored labels only for 9.0 | 28 local-only PRMs: 23 YL-Clarity 9.0 structural files plus five 9.1 files with same-run composite curve exports | Yes, local-only completed files | 28 files / 353,830 B smallest | 9.0 structural records preserved; five distinct 9.1 source/export pairs validate all 138,000 time/signal points; direct PRM peaks remain unsupported |
...
23 YL-Clarity 9.0.1.19 completed PRMs for exact structural records plus five distinct
9.1.0.76 PRMs paired to composite full curves. Exact typed-property, duplicate-gzip,
record-count, label and finite-binary32 gates are shared, while all 138,000 paired 9.1
points separately validate the narrow scientific time/signal semantics. The 9.0 profile
remains structural-only and direct PRM peaks remain unsupported. See
