...
DGX environment, verifies the exact wheel in clean CLI and GUI environments, publishes
the same immutable bytes through TestPyPI and PyPI using Trusted Publishing, and attaches
those same bytes and `SHA256SUMS.txt` to the GitHub Release only after all identity gates
pass. No native standalone binary is part of this release.
