...
def test_package_version() -> None:
    assert ordifile.__version__ == "0.3.1"
...
def test_built_wheel_contains_only_ordifile_package_and_entry_points(tmp_path: Path) -> None:
    ...
    assert "Requires-Dist: pyside6-essentials<6.12,>=6.11.2; extra == 'gui'" in metadata
    assert "Requires-Dist: pyside6-essentials<6.12,>=6.11.2\n" not in metadata
