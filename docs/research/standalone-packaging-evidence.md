...
| [`vswhere` Find VC](https://github.com/microsoft/vswhere/wiki/Find-VC), [Visual Studio versions](https://github.com/microsoft/vswhere/wiki/Versions), [Build Tools component IDs](https://learn.microsoft.com/en-us/visualstudio/install/workload-component-id-vs-build-tools), [MSVC version mapping](https://learn.microsoft.com/en-us/cpp/overview/compiler-versions), [command-line C++ tools](https://learn.microsoft.com/en-us/cpp/build/building-on-the-command-line), and [developer shells](https://learn.microsoft.com/en-us/visualstudio/ide/reference/command-prompt-powershell) | Microsoft's discovery guidance uses `vswhere -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64`; Visual Studio major 17 identifies 2022, MSVC 19.30 maps to its initial compiler, and the installed developer command files configure compiler and SDK variables for x64 native builds. | Prove an already active x64 MSVC 19.30+ environment with compiler/linker/resource/SDK/PE/`dumpbin`/MSBuild/execution checks. Only when no compiler is active, activate one registered VS 2022 toolchain, mask its values, export a fixed allowlist to the job, and repeat the full probe in the next step. |
| [Visual Studio 2022 release history](https://learn.microsoft.com/en-us/visualstudio/releases/2022/release-history), [command-line installer parameters](https://learn.microsoft.com/en-us/visualstudio/install/use-command-line-parameters-to-install-visual-studio), [deployment exit codes](https://learn.microsoft.com/en-us/visualstudio/install/deploy-a-layout-onto-a-client-machine), and [Build Tools license terms](https://visualstudio.microsoft.com/license-terms/vs2022-ga-diagnosticbuildtools/) | Microsoft publishes fixed VS 2022 bootstrapper assets and documents administrator, component, no-restart, exit-code, and licensing boundaries. Exit 3010 requires a reboot before use; Build Tools use requires an applicable Visual Studio license or eligibility. | Keep installation out of the reusable build. A private maintenance operation may use only a reviewed fixed asset, exact minimal components, existing administrator authority, no automatic restart, and a documented applicable license; 3010 stops before validation. |
| [Nuitka 4.1.3 Windows compiler selection](https://github.com/Nuitka/Nuitka/blob/4.1.3/nuitka/build/SconsUtils.py) and [executable lookup](https://github.com/Nuitka/Nuitka/blob/4.1.3/nuitka/utils/Execution.py) | The pinned backend directly uses an active `cl` found on `PATH` and skips its Visual Studio setup discovery in that case. | Require the active compiler probe to pass whenever `cl` is found; use registered Visual Studio activation only when active `cl` is absent, and never persist the activated environment beyond the job. |
