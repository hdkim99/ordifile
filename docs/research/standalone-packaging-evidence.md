...
| [PySide6 6.11.2 package initializer](https://github.com/pyside/pyside-setup/blob/v6.11.2/sources/pyside6/PySide6/__init__.py.in) and [Nuitka import controls](https://github.com/Nuitka/Nuitka/blob/4.1.3/nuitka/options/OptionParsing.py) | The Windows initializer conditionally imports `QtNetwork` only when its optional OpenSSL directory exists and catches `ImportError`; Nuitka provides exact no-follow and Qt plugin-family boundaries. A full direct-Nuitka build demonstrated that static following can otherwise add a prohibited network runtime even though Ordifile imports only QtCore, QtGui and QtWidgets. | Apply `--nofollow-import-to=PySide6.QtNetwork` plus separate TLS/network-information plugin exclusions only to Windows PySide/full builds, keep the macOS spec unchanged, and require both the minimal PySide bundle audit and final extracted scientific/window smoke. |
