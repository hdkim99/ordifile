...
|---|---|---|---|---|---|---|
| [YOUNG IN Chromass Track C](youngin-chromass-format-investigation.md) | YOUNG IN Chromass and DataApex official material plus owner-supplied local intake | Stored FID/TCD labels; scientific detector semantics unresolved | 23 local-only YL-Clarity 9.0.1.19 `.prm` files plus 2 paired Result Tables; no `.raw` or chromatogram curve oracle | Yes, local-only completed files | 23 files / 353,830 B smallest | Exact structural profile and raw binary32 records validated; direct scientific signal remains blocked by the missing same-run curve oracle |
...
intake supplies 23 YL-Clarity 9.0.1.19 completed PRM files. Exact typed-property,
duplicate-gzip, record-count, label and finite-binary32 gates now support an
Experimental raw-record adapter. Two paired Result Tables establish the separate peak-result
path; the lack of a same-run chromatogram curve blocks direct scientific
time/scaling/unit semantics, not deterministic structural conversion. See
